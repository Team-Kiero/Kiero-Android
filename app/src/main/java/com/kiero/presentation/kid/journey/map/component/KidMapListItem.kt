        ...
        Column {
            Row(
                ...
                Text(
                    text = item.name,
                    color = when {
                        item.isOngoing || item.status == MapScheduleStatus.PENDING -> KieroTheme.colors.white
            ...
            Text(
                ...
                color = when {
                    ...
                    item.status == MapScheduleStatus.PENDING -> KieroTheme.colors.gray500
        ...
        Column(
            ...
            KidMapListItem(
                item = KidMapItemModel(
                    ...
                    status = MapScheduleStatus.COMPLETE
                )
            )
            // 실패 - 라인 없음, 흐리게
            KidMapListItem(
                item = KidMapItemModel(
                    ...
                    isOngoing = false,
                    stoneType = KidJourneyStoneType.COURAGE,
                    status = MapScheduleStatus.FAILED
                )
            )
            // 진행중 - 라인 있음, main 색상
            KidMapListItem(
                item = KidMapItemModel(
                    ...
                    status = MapScheduleStatus.PENDING
                )
            )
